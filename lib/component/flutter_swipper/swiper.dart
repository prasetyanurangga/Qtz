...
class _TinderState extends _CustomLayoutStateBase<_TinderSwiper> {
  ...
  void afterRender() {
    ...
    _startIndex = -3;
    _animationCount = 5;
    opacity = [0.0, 0.5, 0.5, 1.0, 0.0, 0.0];
...
class _StackViewState extends _CustomLayoutStateBase<_StackSwiper> with TickerProviderStateMixin {
  List<double> scales;
  List<double> offsets;
  List<double> opacity;
  @override
  ...
  void _updateValues() {
    if (widget.scrollDirection == Axis.horizontal) {
      ...
      offsets = [-space, -space / 2 * 2, -space / 2, 0.0, _swiperWidth];
    } else {
